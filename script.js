...
function showSection(sectionId) {
  ...
  if (targetSection) {
    ...
    console.warn(
      `Section with id '${sectionId}' not found. Defaulting to dashboard.`
    );
...
function registerForEvent(eventId) {
  alert(
    `Registration for event ${eventId} would be handled here. This is a demo.`
  );
}
  ...
  fetch("http://localhost:3000/api/events")
    ...
    .then((data) => {
      ...
      if (dashboardEventsContainer) {
        dashboardEventsContainer.innerHTML = allFetchedEvents
          .slice(0, 3)
          .map(createEventCard)
          .join("");
      }
      ...
      if (allEventsPageContainer) {
        allEventsPageContainer.innerHTML = allFetchedEvents
          .map(createEventCard)
          .join("");
      }
      ...
      if (categoryFilter) {
        if (!categoryFilter.dataset.listenerAttached) {
          categoryFilter.addEventListener("change", () =>
            displayFilteredEvents(allFetchedEvents)
          );
...
function displayFilteredEvents(eventsToFilter) {
  ...
  if (allEventsPageContainer) {
    if (filteredEvents.length > 0) {
      allEventsPageContainer.innerHTML = filteredEvents
        .map(createEventCard)
        .join("");
    } else {
      allEventsPageContainer.innerHTML =
...
function loadCouncilInfo() {
  fetch("http://localhost:3000/api/council")
    ...
    .then((data) => {
      ...
      if (councilContainer) {
        if (data.length > 0) {
          councilContainer.innerHTML = data
            .map(createCouncilMemberCard)
            .join("");
        } else {
          councilContainer.innerHTML =
            "<p>Student council information is not available.</p>";
        }
      }
    })
    .catch((error) => {
      ...
      if (councilContainer)
        councilContainer.innerHTML =
          "<p>Could not load council information. Please try again later.</p>";
    });
}
...
function createPaperCard(paper) {
  ...
  const studentYearInfo = paper.studentYear
    ? `<p><strong>Student Year Level:</strong> ${paper.studentYear}</p>`
    : "";
...
function loadPapers() {
  fetch("http://localhost:3000/api/papers")
    ...
    .then((data) => {
      ...
      if (papersContainer) {
        if (allFetchedPapers.length > 0) {
          papersContainer.innerHTML = allFetchedPapers
            .map(createPaperCard)
            .join("");
        } else {
          papersContainer.innerHTML =
            "<p>No exam papers available at the moment.</p>";
        }
      }
      ...
      if (yearFilter && !yearFilter.dataset.listenerAttached) {
        yearFilter.addEventListener("change", () =>
          displayFilteredPapers(allFetchedPapers)
        );
    ...
    .catch((error) => {
      ...
      if (papersContainer)
        papersContainer.innerHTML =
          "<p>Could not load exam papers. Please try again later.</p>";
    });
}
  ...
  const filteredPapers =
    yearLevel === "all"
      ...
      : papersToFilter.filter(
          (paper) =>
            paper.studentYear && paper.studentYear.toString() === yearLevel
        );
...
function loadHiring() {
  fetch("http://localhost:3000/api/hiring")
    ...
    .catch((error) => {
      ...
      if (hiringContainer)
        hiringContainer.innerHTML =
          "<p>Could not load hiring information. Please try again later.</p>";
    });
}
...
function applyForJob(company, position) {
  alert(
    `Application for ${position} at ${company}. thanks for applying. This is a demo.`
  );
}
